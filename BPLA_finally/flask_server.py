...
def check_session_token(token):
    if not token:
        logging.warning('Токен отсутствует, перенаправление на страницу логаута')
        return redirect(url_for('logout'))  # Перенаправление на страницу логаута
    username = verify_token(token)
    if not username:
        logging.warning('Недействительный токен, перенаправление на страницу логаута')
        return redirect(url_for('logout'))  # Перенаправление на страницу логаута
        ...
        with DBConnectionManager(factory, path_to_db='prod.db') as conn:
            ...
            if request.method == 'POST':
                try:
                    ...
                    logging.error(f'Произошла ошибка при добавлении дрона: {str(e)}')
...
def generate_telemetry():
    ...
    delta_longitude = distance / (111320 * np.cos(np.radians(center_latitude)))  # Корректируем по широте

    # Генерация новых координат
    current_latitude = center_latitude + delta_latitude * np.random.choice([-1, 1])
    current_longitude = center_longitude + delta_longitude * np.random.choice([-1, 1])
    ...
    battery_level -= (speed * flight_time / 3600) * (100 / 20)  # Пропорционально скорости и времени
    battery_level = max(battery_level, 0)  # Уровень заряда не может быть меньше 0%
